...
require_once __DIR__ . '/../infrastructure/entrypoint/web/presentation/Flash.php';
require_once __DIR__ . '/../infrastructure/entrypoint/web/presentation/View.php';
require_once __DIR__ . '/../infrastructure/entrypoint/web/controller/config/WebRoutes.php';
...
$routes = WebRoutes::getRouter();
if (!isset($routes[$route])) {
    http_response_code(404);
    View::render('home', ['message' => 'Ruta no encontrada.']);
    exit;
}
...
if ($httpMethod !== $definition['method']) {
    ...
    View::render('home', ['message' => 'Método HTTP no permitido.']);
    exit;
}
    ...
    switch ($definition['action']) {
        ...
        case 'forgot.send':
            ...
            if ($forgotEmail === '' || !filter_var($forgotEmail, FILTER_VALIDATE_EMAIL)) {
                Flash::setError(array('email' => 'Introduce un correo electrónico válido.'));
                Flash::setOld(array('email' => $forgotEmail));
                View::redirect('auth.forgot');
            }
            $repository = DependencyInjection::getUserRepository();
            $foundUser = $repository->getUserByEmail(new UserEmail($forgotEmail));

            // Always show generic success to avoid user enumeration.
            if ($foundUser !== null && $foundUser->getStatus() === UserStatusEnum::ACTIVE) {
                // Generate a secure temporary password (10 hex chars ≥ 8 chars).
                $tempPassword = bin2hex(random_bytes(5));
                $newPassword = UserPassword::fromTextPlain($tempPassword);
                $updatedUser = $foundUser->updatePassword($newPassword);
                $repository->update($updatedUser);
                sendPasswordRecoveryEmail(
                    $foundUser->getUserEmailValue(),
                    $foundUser->getUserNameValue(),
                    $tempPassword
                );
            }
